...
export async function buildTreeSnapshot(input: BuildTreeSnapshotInput): Promise<TreeSnapshot | undefined> {
	...
	const children: TreeChild[] = manifest.specs.map((spec, index) => {
		...
		let stale = false;
		if (state === "running") {
			const bestTimestamp = activityRecord?.updatedAt ?? startedAt ?? manifest.createdAt;
			const updatedMs = Date.parse(bestTimestamp);
		...
		for (const spawnRecord of spawns) {
			...
			for (const spawnChild of spawnRecord.children) {
				...
				const bestChildTimestamp = spawnChild.updatedAt || spawnChild.startedAt || spawnRecord.createdAt;
				const childUpdatedMs = Date.parse(bestChildTimestamp);
