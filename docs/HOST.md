...
| `dstack_result` | Bounded summary. Root reads are always nonblocking and normally happen after the completion notification; `waitSeconds` is ignored there so a read can never pin the root turn and lock the user out. Depth-1 collection waits for a running nested task, defaulting to one supervision interval and bounded by `waitSeconds`. Full detail is opt-in. |
...
Structured dmode launches carry `workflow` metadata with `playbook`, `assignment`, `phase`, `completedPhases`, and `artifacts`. Root launches depth-1 `poteto-agent` owners for nontrivial work: one owner for related outcomes, or one owner per outcome in a single parallel `tasks[]` batch when one user turn contains multiple genuinely independent outcomes. Independent read-only or non-overlapping worker slices default to parallel `tasks[]` calls. Parallel writers each use a separate worktree (`worktree: true`). Owners may submit repeated worker batches. Workers and reviewers are terminal, and only owners read the selected playbook. The file scheduler caps all root and nested child processes at the session's persisted `scheduler.totalSlots` capacity (default 12).
