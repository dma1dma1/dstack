...
Produce a phased implementation plan grounded in the **Principles** section of the `dmode` skill. The plan is the deliverable. Do not implement.
...
Read the **Principles** section of the `dmode` skill end to end, and the leaf `principle-*` skills it indexes. The principles govern every plan decision; cross-link them.

## 2. Scope and constraints

State your read of scope and constraints in one paragraph. Use `dstack_ask` only for genuinely ambiguous intent (the **never-block-on-the-human** principle skill); give concrete options with each open question.
...
- Prefer `agent: "poteto-agent"`. `general-purpose` is the fallback. Never use the built-in `plan` agent; it ignores this skill.
- Pass `model:` explicitly per the configured roles (defaults inherit-parent unless models.json sets the role).
...
If a phase creates or edits a skill, the phase instructs the implementer to use the **create-skill** skill (the host's built-in for authoring SKILL.md files).
...
- Browser / Electron / Web UIs: the `control-ui` skill from the `host companion` plugin.
- CLIs and TUIs: the `control-cli` skill from the `host companion` plugin.
...
In the overview, name which dmode non-negotiables the implementer must apply, by name:
