...
3. Plan the fix from the trace. Use Architect only if the trace leaves a consequential unresolved structural choice. Delegate implementation with structured workflow metadata and the trace or design artifact paths. Review the combined diff yourself. Capture a post-fix trace.
