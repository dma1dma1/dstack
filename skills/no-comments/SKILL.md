...
1. Spawn `dstack_task` with `agent: "comment-sicko"`. Pass the scope. Do not restate its rules. `dstack_task` returns a `taskId` immediately; the parent may do independent work. Wait for the background completion notification without polling, then call `dstack_result` once with `taskId`.
