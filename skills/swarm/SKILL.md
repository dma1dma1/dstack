...
4. Pick the worker model from `swarm workers` in `~/.pi/agent/dstack/models.json` when present. Otherwise inherit-parent. For a model race, name each arm's model up front.
