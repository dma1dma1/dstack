...
The parent finds its own transcript file before fanning out. The system prompt names the active workspace's `agent-transcripts/` directory; use that path. Do not glob across `host transcript dirs/`. That crosses workspace boundaries and reads private chats from unrelated projects.
...
|---|---|---|
| Judgment | your configured reflect-judgment model (or inherit-parent if unset) | `references/judgment-reviewer.md` |
...
- Substantive existing-skill edit (a new section, a new pattern table, more than ~10 lines): hand to the host's built-in `create-skill` skill and run its draft / test / iterate loop.
