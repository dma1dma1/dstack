...
This skill orchestrates three others: an inline mining pass (see step 1), the host's built-in `create-skill` (authoring), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.

## Flow

### 0. Check for an existing skill

Look recursively for `project skills/**/*-mode/SKILL.md` and `user skills/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`project skills/<handle>/`), not only at the top level. If one exists, confirm intent with `dstack_ask` (unless they already said "update my skill" or similar):
...
Locate the active workspace's transcripts before fanning out. The system prompt names the workspace's `agent-transcripts/` directory. Use only that path. Don't glob across `host transcript dirs/`. That crosses workspace boundaries and reads private chats from unrelated projects.
...
Mining misses intent that hasn't come up yet. Use the `dstack_ask` tool (structured multi-choice) rather than asking the user to type from scratch. Lower cognitive load, higher hit rate.
...
The **dmode** skill shows the shape. Read it for granularity. Don't copy its content; the user's rules are not the same as dmode's.

### 4. Draft the skill

Use the host's built-in `create-skill` skill to author the skill. Placement:
...
- The **dmode** skill: example of the output shape.
